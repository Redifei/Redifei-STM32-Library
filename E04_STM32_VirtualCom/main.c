/*
 main.c - main for stm32_virtualCom
...
int main(void) {
  ...
  red_vcomPort_t* redVcom = redVcomInit(RED_VCOM_PORT, &redVcomUserSetting);
  ...
  while (1) {
    ...
    if (redVcom->available(redVcom)) {
      redVcom->printf(redVcom, "%d : %c\r\n", i++, redVcom->getChar(redVcom));
    }
    delay(100);
  }
}
